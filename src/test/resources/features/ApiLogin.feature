...
Scenario Outline: Verify the Login API With valid credentials 
	...
	When User makes POST request on Login API
	Then Status code for Security API returns <status_code>
    And Succesful login return a message <successMsg>
    Examples: 
        |Test description			    |RequestBody				|status_code |successMsg		  |
        |Test with invalid email format |LoginWithValidCreds.json	|200		 |Login successful	  |
